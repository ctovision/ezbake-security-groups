...
public class EzGroupsGraph implements Closeable {
    ...
    public Object createCommonGroup() {
        ...
        Iterator<Vertex> v = graph.query().has(BaseVertex.INDEX, 0).limit(1).vertices().iterator();
        if (!v.hasNext()) {
            try {
                // create the common group
                Group cg = framedGraph.addVertex(null, Group.class);
                cg.setName(Group.COMMON_GROUP);
                cg.setGroupName(Group.COMMON_GROUP);
                cg.setIndex(idProvider.nextID());
    ...
    protected Group addGroup(BaseVertex.VertexType ownerType, String ownerPrincipal, String groupName, Object parentGroup) throws VertexExistsException, UserNotFoundException, AccessDeniedException, IndexUnavailableException {
        return addGroup(ownerType, ownerPrincipal, groupName, graph.getVertex(parentGroup), new GroupInheritancePermissions(true, false, false, false, false), new UserGroupPermissionsWrapper(true, true, true, true, true), true, false);
    }
    ...
    private Group addGroupPrivileged(Vertex parent, String groupName, GroupInheritancePermissions inheritance,
        ...
        if (siblingExists.hasNext()) {
            ...
            throw new VertexExistsException(parentGroup.getGroupName() + " already has child with name: " + groupName + "(ID: " + siblingExists.id() + ")");
        }
    ...
    public Set<Group> getGroups(Set<String> groupNames) {
        ...
        for (String groupName : groupNames) {
            Iterator<Group> git = framedGraph.query().has(Group.GROUP_NAME, groupName).vertices(Group.class).iterator();
            if (git.hasNext()) {
                groups.add(git.next());
            }
        }
     ...
     * @return
     */
    public User addUser(BaseVertex.VertexType type, String principal, String name) throws InvalidVertexTypeException, VertexExistsException, AccessDeniedException, UserNotFoundException, IndexUnavailableException {
    ...
    public void updateUser(BaseVertex.VertexType type, String principal, String newPrincipal) throws UserNotFoundException, InvalidVertexTypeException {
    ...
    public void setUserActiveOrNot(BaseVertex.VertexType type, String principal, boolean active) throws InvalidVertexTypeException, UserNotFoundException {
        logger.info("Processing {} User user type: {}, principal: {}", type, principal, active?"activate":"deactivate");
        if (type != BaseVertex.VertexType.APP_USER && type != BaseVertex.VertexType.USER) {
            throw new InvalidVertexTypeException("Cannot add vertex of type: " + type + " to groups");
        }
     ...
     * @param type whether the user is a USER or an APP_USER
     * @param principal user principal
     * @param groupId Vertex ID of the group the user should be added to
     */
    public void addUserToGroup(BaseVertex.VertexType type, String principal, Object groupId) throws UserNotFoundException, InvalidVertexTypeException {
        addUserToGroup(type, principal, groupId, true, false, false, false, false);
    }
     ...
     * @param groupId Vertex ID of the group the user should be added to
     */
    public void addUserToGroup(BaseVertex.VertexType type, String principal, Object groupId, boolean dataAcces, boolean adminRead, boolean adminWrite, boolean adminManage, boolean adminCreateChild) throws UserNotFoundException, InvalidVertexTypeException {
        logger.info("Processing addUserToGroup group: {}, user type: {}, principal: {}", groupId, type, principal);
        if (type != BaseVertex.VertexType.APP_USER && type != BaseVertex.VertexType.USER) {
            throw new InvalidVertexTypeException("Cannot add vertex of type: " + type + " to groups");
        }
        FramedGraph<TitanGraph> framedGraph = framedGraphFactory.create(graph);
    ...
    public void removeUserFromGroup(BaseVertex.VertexType type, String principal, String groupName) throws UserNotFoundException, VertexNotFoundException {
        // Get the group verted
        Iterator<Vertex> groups = graph.query().has(Group.GROUP_NAME, groupName).limit(1).vertices().iterator();
        if (!groups.hasNext()) {
            throw new VertexNotFoundException("Cannot find the group: " + groupName);
     ...
     */
    public void removeUserFromGroup(BaseVertex.VertexType type, String principal, Object groupId) throws UserNotFoundException {
        logger.info("Processing removeUserFromGroup group: {}, user type: {}, principal: {}", groupId, type, principal);
        if (type != BaseVertex.VertexType.APP_USER && type != BaseVertex.VertexType.USER) {
            throw new NotImplementedException("Cannot remove vertex of type: " + type + " to groups");
        }
    ...
    public Set<User> groupMembers(BaseVertex.VertexType userType, String userId, String groupName, boolean explicityOnly) throws VertexNotFoundException, UserNotFoundException {
        return groupMembers(userType, userId, groupName, explicityOnly, true, true);
    }
