...
public class EzGroupsService extends EzBakeBaseThriftService implements EzGroups.Iface {
    ...
    public TProcessor getThriftProcessor() {
        ...
        EzGroupsService instance = Guice.createInjector(new EzGroupsModule(p)).getInstance(EzGroupsService.class);
    ...
    @Override
    public void createGroup(EzSecurityToken ezSecurityToken, String parent, String name,
                            GroupInheritancePermissions inheritance
    ) throws EzSecurityTokenException, EzGroupOperationException, AuthorizationException {
        createGroupWithInclusion(ezSecurityToken, parent, name, inheritance, true, false);
    }

    @Override
    public void createGroupWithInclusion(EzSecurityToken ezSecurityToken, String parent, String name,
        ...
        try {
            ...
            // Create the group
            graph.addGroup(userTypeFromToken(ezSecurityToken), principal, name, parent, inheritance,
    ...
    public void modifyUser(EzSecurityToken ezSecurityToken, String principal, String newPrincipal) throws EzSecurityTokenException, EzGroupOperationException {
        validateToken(ezSecurityToken, true);
        modifyUserOfType(BaseVertex.VertexType.USER, principal, newPrincipal);
    }
    ...
    @Override
    public void createAppUser(EzSecurityToken ezSecurityToken, String securityID, String name) throws EzSecurityTokenException, EzGroupOperationException, AuthorizationException {
        validateToken(ezSecurityToken, true);

        try {
            // Create the app user. root.app.name and root.appaccess.name will be created
            graph.addUser(BaseVertex.VertexType.APP_USER, securityID, name);

            String appGroup = getNamespacedAppGroup(name, null);
            ...
            logger.error("Cannot create user, the vertex already exists! user name: {}", securityID);
    ...
    public Set<Long> createAppUserAndGetAuthorizations(EzSecurityToken token, List<String> chain, String securityId, String appName) throws EzSecurityTokenException, AuthorizationException, EzGroupOperationException, GroupQueryException {
        ...
        User user;
        try {
            user = graph.addUser(BaseVertex.VertexType.APP_USER, securityId, appName);
        } catch (InvalidVertexTypeException|AccessDeniedException|IndexUnavailableException|UserNotFoundException e) {
            logger.error("creatAppUserAndGetAuths: Unable to create user {}", e.getMessage());
            throw new EzGroupOperationException("Unable to create user: "+e.getMessage(),
                    OperationError.UNRECOVERABLE_ERROR);
        } catch (VertexExistsException e) {
            logger.info("user already exists, getting their group authorizations list");
            // Ok if user already exists... get the user
            try {
                user = graph.getUser(BaseVertex.VertexType.APP_USER, securityId);
        ...
        authorizations.add(user.getIndex());
    ...
    public void modifyAppUser(EzSecurityToken ezSecurityToken, String securityID, String newSecurityID, String newName) throws TException {
        validateToken(ezSecurityToken, true);
        modifyUserOfType(BaseVertex.VertexType.APP_USER, securityID, newSecurityID);
    }
    ...
    public AllGroupMembers getGroupMembers(EzSecurityToken ezSecurityToken, String groupName, boolean explicitMembers) throws EzSecurityTokenException, GroupQueryException {
        validateToken(ezSecurityToken);
        String realGroupName = addRootGroupPrefix(groupName);
    ...
    public Set<UserGroup> getUserGroups(EzSecurityToken token, boolean explicitGroups) throws GroupQueryException, EzSecurityTokenException {
        ...
        for (Group g : groups) {
            ...
            group.setGroupName(removeRootGroupPrefix(g.getGroupName()));
    ...
    public Set<UserGroup> getAppUserGroups(EzSecurityToken token, boolean explicitGroups) throws EzSecurityTokenException, GroupQueryException {
        ...
        // TODO: check requesting apps groups? I don't think I need to in this case because this isn't meant to be shared
        ...
        try {
            ...
            try {
                ...
                throw new EzGroupOperationException("Unable to create user: "+userId+" reason: "+e1.getMessage(),
                        OperationError.UNRECOVERABLE_ERROR);
            } catch (InvalidVertexTypeException|UserNotFoundException|VertexExistsException e1) {
    ...
    public void deactivateGroup(EzSecurityToken ezSecurityToken, String groupName, boolean andChildren) throws EzSecurityTokenException, EzGroupOperationException, AuthorizationException {
        ...
        String principal = ezSecurityToken.getTokenPrincipal().getPrincipal();
        try {
            graph.setGroupActiveOrNot(userTypeFromToken(ezSecurityToken), principal, addRootGroupPrefix(groupName), false, andChildren);
        } catch (VertexNotFoundException e) {
    ...
    public void activateGroup(EzSecurityToken ezSecurityToken, String groupName, boolean andChildren) throws EzSecurityTokenException, EzGroupOperationException, AuthorizationException {
        ...
        String principal = ezSecurityToken.getTokenPrincipal().getPrincipal();
        try {
            graph.setGroupActiveOrNot(userTypeFromToken(ezSecurityToken), principal, addRootGroupPrefix(groupName), true, andChildren);
        } catch (VertexNotFoundException e) {
            logger.error("No group found. Cannot deactivate group {}", groupName);
            throw new EzGroupOperationException("Unable to deactivate group:" + groupName,
                    OperationError.GROUP_NOT_FOUND);
    ...
    public void removeUserFromGroup(EzSecurityToken ezSecurityToken, String groupName, String userId) throws EzSecurityTokenException, AuthorizationException, EzGroupOperationException {
        ...
        try {
            ...
            try {
                throw new EzGroupOperationException("No group found for name: " + groupName,
                        OperationError.GROUP_NOT_FOUND);
            } catch (EzGroupOperationException e1) {
                e1.printStackTrace();
            }
        }
    }
    ...
    public Set<Long> getGroupsMask(EzSecurityToken ezSecurityToken, Set<String> groupNames) throws EzSecurityTokenException, EzGroupOperationException {
        ...
        // Get all the groups
        groupNames = addRootGroupPrefix(groupNames.toArray(new String[groupNames.size()]));
    ...
    private void ensureUserHasAdminGroup(BaseVertex.VertexType userType, String userId, String groupName,
        ...
        try {
            if (!graph.userPermissionsOnGroup(userType, userId, groupName).contains(adminType)) {
                err = userId + " cannot add other users to "+removeRootGroupPrefix(groupName)+". user does not have permission";
            }
        } catch (UserNotFoundException|VertexNotFoundException e) {
            err = userId + " cannot add other users to "+removeRootGroupPrefix(groupName) + " : " + e.getMessage();
        }
    ...
    private Set<Long> getAuthorizations(BaseVertex.VertexType userType, String userId, List<String> appFilterChain) throws GroupQueryException {
        Set<Long> auths = Collections.emptySet();

        // Only get auths if the user exists
        try {
            if (!graph.getUser(userType, userId).isActive()) {
        ...
        if (userType == BaseVertex.VertexType.USER) {
            // Split groups into 2 sets - those that users always have (even if app doesn't) and those that users only have if app has too
            Set<Long> groupsUserHasRegardless = Sets.newHashSet();
            ...
            // What to do here?
            Set<Long> appAuths = Sets.newHashSet();
            for (Group g : userGroups) {
     ...
     */
    private void modifyUserOfType(BaseVertex.VertexType type, String id, String newId) throws EzGroupOperationException {
        try {
            graph.updateUser(type, id, newId);
        } catch (InvalidVertexTypeException e) {
    ...
    private static String addRootGroupPrefix(String unprefixed) {
    ...
    private static Set<String> addRootGroupPrefix(String... unprefixeds) {
    ...
    private static String removeRootGroupPrefix(String prefixed) {
    ...
    private static String getNamespacedAppGroup(String appName, String appGroup) {
        return Joiner.on(EzGroupNamesConstants.GROUP_NAME_SEP).skipNulls().join(EzGroupNamesConstants.ROOT,
                EzGroupNamesConstants.APP_GROUP, appName, appGroup);
    }
